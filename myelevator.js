...
Elevator.prototype.decide = function () {
    ...
    var lengthRequest = requests.length;
    var currentFloor = elevator.at_floor(); // [refactor] add currentFloor 
    var lastDecision = Math.floor(num_floors / 2); // [refactor] add lastdecision default num_floors / 2
    ...
    for (var i = 0; i < lengthRequest; i++) {
        var handled = false;
        for (var j = 0; j < elevators.length; j++) {
            if (elevators[j].get_destination_floor() == requests[i]) {
                handled = true;
                lastDecision = lengthRequest !== (i+1) ? requests[i+1] : requests[i-1]; // refer to next request for last decision
                break;
            }
        }
        if (!handled) {
            var closestFloorRequest = requests.reduce(function (prev, curr) { // for get closest floor request from current floor
                return (Math.abs(curr - currentFloor) < Math.abs(prev - currentFloor) ? curr : prev);
            });
            return this.commit_decision(closestFloorRequest);
        }
    }
    return this.commit_decision(lastDecision);
};
